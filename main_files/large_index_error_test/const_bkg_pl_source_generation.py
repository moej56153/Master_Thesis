...
data_path_d = "./main_files/large_index_error_test/bkg_20000_pl_source_i_8_K_8__4"

if not os.path.exists(f"{data_path_d}"):
    os.mkdir(f"{data_path_d}")
    
with open(f"{data_path_d}/source_params.pickle", "wb") as f:
    pickle.dump((source_ra, source_dec, source_piv, source_index, source_K), f)
...
background_counts["COUNTS"] = np.full(background_counts["COUNTS"].shape, 20000.)
...
def calc_count_rates(resp_mats, ra, dec, index, piv, K):
    s = Powerlaw()
    s.index = index
    s.piv = piv
    s.K = K
...
source_counts = calc_count_rates(resp_mats, source_ra, source_dec, source_index, source_piv, source_K)
